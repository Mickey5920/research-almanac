...
Daily rhythms are varied workflow suggestions saved with the result, not auspicious appointments. The six overview groups cover humanities, social sciences, natural sciences, engineering, agriculture and medical research. The local catalog adds 14 lookup categories, 38 direction templates, 12 methods and 9 research stages, plus a general fallback. These direction templates are not an exhaustive official degree-program catalog. Combine up to four directions and four explicitly selected methods; preserve unlisted specialties through `custom_specialties`. Academic/professional master’s and doctoral labels are metadata, not automatic workload assumptions. Daily checklists open by default. The six overview groups link to their detailed daily plans, including links from weekly cells. Use the saved nine-type activity catalog for concrete reading, figure-making, synthesis, thinking, reproduction, experiments, code, writing and discussion steps. Show at most two daily activities plus a collapsible optional library; adapt execution-oriented activities to the research stage. Save a weekly focus with priorities, outputs, questions and activity dates, preserving the user’s own focus. These are project-authored workflow suggestions. Never convert suggested actions into booked tasks without known durations and real project constraints. No arbitrary luck scores, fabricated Qimen charts or medical treatment advice. Reference existing protocols for fieldwork, clinical commitments and experiments.
