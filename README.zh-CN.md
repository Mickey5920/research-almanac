...
**0.7.0 版本 · 本地运行 · Node.js 22+ · 中英文文档与命令行报告**
...
每个新投稿窗口保存点击时刻星历，并结合用户自填星座生成个人择时分析；使用 Astronomy Engine 2.1.19 本地计算，HTML 仅展示已保存记录。可选填写个人星座，例如在 Agent 对话中说“我的星座是处女座，加入星象参考”，无需生日或出生地。省略个人星座仍可查看窗口星象；不填写不影响运行。

输入示例见 [星象示例](examples/astrology.json)：astrology.personal_sign 为中文星座名称，astrology.enabled=false 可关闭本轮星象。回归黄道十二等分，地心当日黄道坐标，支持 1900–2100；水星近留阈值为 0.02°/日。个人星座文化匹配仅作末级排序参考。旧记录不追加推算。
