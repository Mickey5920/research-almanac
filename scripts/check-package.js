...
const required=['SKILL.md','README.md','README.zh-CN.md','LICENSE','package-lock.json','docs/PROJECT-GUIDE.md','docs/IMPLEMENTATION.md','assets/marketing/hero.png','assets/marketing/poster.zh.png'];
for(const p of required)if(!existsSync(join(root,p)))throw new Error('Missing '+p);
const skip=new Set(['node_modules','runs','.git','.cache','dist']);
let count=0,links=0;
