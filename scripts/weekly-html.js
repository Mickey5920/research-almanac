...
export function renderWeeklyMarkdown(report,language='zh'){
 ...
 for(const [i,d] of report.days.entries()){
  ...
  for(const s of report.disciplines){const task=s.week[i];lines.push('- **'+t(s.name)+' · '+t(task.title)+'**: '+t(task.action)+' → '+t(task.output));}
  const rhythm=d.rhythm[language];lines.push('','**'+(zh?'今日节奏':'Daily rhythm')+' · '+rhythm.title+'**',...rhythm.blocks.map(b=>'- '+b.time+' · '+b.title+' · '+b.detail));
